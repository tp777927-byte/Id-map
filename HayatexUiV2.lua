...
if not _G.Theme then
    ...
    -- (hubName ยังไม่ถูก init ตรงนี้ ใช้ชื่อ default แทน)
end
...
--  WINDOW FUNCTION
--  config = {
--    Name  = "ชื่อ Hub",        (default: "HayateX Hub")
--    Tag   = "ค่าย / version",  (default: "")
--    Logo  = "rbxassetid://...", (default: icon เดิม)
--  }
-- ============================================================
function HayateX:Window(config)
    -- รองรับทั้ง :Window("text") และ :Window({...})
    if type(config) == 'string' then
        config = { Tag = config }
    end
    config = config or {}

    local hubName = config.Name  or 'HayateX Hub'
    local hubTag  = config.Tag   or ''
    local hubLogo = config.Logo  or 'rbxassetid://13940080072'

    local isFirstTab  = false
    local currentPage = ''
    local keybind     = Enum.KeyCode.RightControl

    -- อัพเดต toggle button ให้ใช้ logo ที่ตั้งค่า
    toggleBtn.Image = hubLogo
    ...
    titleLabel.Text                 = string.upper(hubName) .. ' |'
